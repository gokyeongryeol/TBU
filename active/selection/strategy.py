...
class Strategy:
    def __init__(self, data, proxy, target, n_MCdrop, percentile, n_filter, **kwargs):
        self.data = data
        self.proxy = proxy
        self.target = target
    ...
    def _medium_constrain(self):
        ...
        if self.percentile >= 0.0:
            ...
            ne_sub_idxs = torch.ones(U, dtype=torch.bool)
    ...
    def constrain(self):
        if self.percentile >= 0.0 or self.n_filter > 0:
